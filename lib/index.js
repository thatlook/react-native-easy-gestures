...
export default class Gestures extends Component {
  ...
  onMoveStart = (event) => {
    const { style } = this.state;
    const { onMultyTouchStart, onStart, rotatable, scalable, draggable } = this.props;
    ...
    if (rotatable || scalable || draggable) {
      onStart(event, style);
  ...
  onMove = (event, gestureState) => {
    ...
    const { onChange, onMultyTouchChange, rotatable, scalable, draggable } = this.props;
